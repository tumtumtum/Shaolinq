		...
		: BaseTests<ComplexPrimaryKeyDataAccessModel>
	{
		...
		public void Test_Complex_Explicit_Joins_With_Include_In_Queryables()
		{
			var query =
				...
				shop in this.model.Shops
				join
				address1 in this.model.Addresses.Include(c => c.Region.Center) on shop.Address equals address1
				join
				address2 in this.model.Addresses.Include(c => c.Region) on shop.SecondAddress equals address2
				join
				_address3 in this.model.Addresses.Include(c => c.Region) on shop.ThirdAddress equals _address3  into g
				from address3 in g.DefaultIfEmpty()
				join
...
#pragma warning restore CS0472 // The result of the expression is always the same since a value of this type is never equal to 'null'
		...
		public void Test_Select_Implicit_Join_On_RelatedObject_And_Other_Related_Object_Of_Same_Type1()
		{
			using (var scope = this.NewTransactionScope())
			{
				var query = this.model.Shops.Include(c => c.Address)
		...
		public void Test_Select_Include_And_Include_RelatedObjects_Via_Pair1()
		{
			using (var scope = this.NewTransactionScope())
			{
				var query = this.model.Shops
					...
					.Include(c => c.Item2.Address.Region)
					.Select(c => c.Item2);
		...
		public void Test_Select_Include_RelatedObject3c()
		{
			using (var scope = this.NewTransactionScope())
			{
				var query = this.model.Shops
					...
					.Include(c => c.x.shopp.Address.Region)
					.Select(c => c.x.shopp);
		...
		public void Test_Include_Basic()
		{
			using (var scope = this.NewTransactionScope())
			{
				var malls = this.model
					.Malls
					.Include(c => c.SisterMall.Address)
					.Include(c => c.SisterMall2)
					.Select(c => new { mall = c.Name})
					.ToList();
			}
		}
		...
		public void Test_Where_With_Condition_Using_Inner_Related_First_Property_Expression()
		{
			using (var scope = this.NewTransactionScope())
			{
				var malls = this.model
					.Malls
					.Where(c => c.Shops.First().Name.Length == 0)
					.ToList();
			}
		}
		...
		public void Test_Include_Collection2a()
		{
			using (var scope = this.NewTransactionScope())
			{
				...
				var shops = mall.SisterMall.Shops.Items().ToList();
