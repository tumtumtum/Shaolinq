...
{
	public abstract class SqlOldDatabaseCreator
		: OldDatabaseCreator
	{
		private static readonly ILog Log = LogManager.GetLogger(typeof(SqlOldDatabaseCreator).Name);
		...
		protected SqlOldDatabaseCreator(SystemDataBasedDatabaseConnection databaseConnection, DataAccessModel model)
		{
		...
		public virtual IEnumerable<string> GetCreateStrings(CreateDatabaseContext createDatabaseContext, TypeDescriptor typeDescriptor)
		{
			...
			foreach (var typeRelationshipInfo in typeDescriptor.GetRelationshipInfos())
			{
				if (typeRelationshipInfo.EntityRelationshipType == EntityRelationshipType.ChildOfOneToMany)
				{
					...
					var valueRequiredAttribute = typeRelationshipInfo.ReferencingProperty.PropertyInfo.GetFirstCustomAttribute<ValueRequiredAttribute>(false);
					...
					{
						this.schemaWriter.AppendForeignKeyColumnDefinition(typeRelationshipInfo.ReferencingProperty.PersistedName + primaryKeyProperty.PersistedShortName, typeRelationshipInfo.RelatedTypeTypeDescriptor, builder, valueRequired);
					}
			...
			{
				var relatedPropertyTypeDescriptor = this.typeDescriptorProvider.GetTypeDescriptor(typeRelationshipInfo.ReferencingProperty.PropertyType);
				...
				var foreignKeys = typeRelationshipInfo
					.RelatedTypeTypeDescriptor.PrimaryKeyProperties
					.Convert<PropertyDescriptor, string>(x => typeRelationshipInfo.ReferencingProperty.PersistedName + x.PersistedShortName)
					.JoinToString(", ");
