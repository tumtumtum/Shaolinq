...
namespace Shaolinq
{
	public static class MethodInfoFastRef
	{
		...
		public static readonly MethodInfo DataAccessObjectExtensionsIncludeMethod = typeof(DataAccessObjectExtensions).GetMethods(BindingFlags.NonPublic | BindingFlags.Static).Single(c => c.Name == "Include" && c.IsGenericMethod && c.GetParameters().Length == 2 && c.ReturnType == c.GetGenericArguments()[0]);
