...
namespace Shaolinq.Postgres.Shared
{
		...
		: DefaultSqlDataTypeProvider
	{
		...
		public PostgresSqlDataTypeProvider(bool nativeUuids, DateTimeKind dateTimeKindIfUnspecified)
		{
			...
			{
				DefineSqlDataType(new PostgresDateTimeDataType(false, dateTimeKindIfUnspecified));
				DefineSqlDataType(new PostgresDateTimeDataType(true, dateTimeKindIfUnspecified));
			}

			if (nativeUuids)
			{
				DefineSqlDataType(new PostgresUuidSqlDataType(typeof(Guid)));
				DefineSqlDataType(new PostgresUuidSqlDataType(typeof(Guid?)));
			}
		}
	}
